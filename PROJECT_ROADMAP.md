...
**P6D IN PROGRESS. Checkpoint 1 Expense READ Foundation is COMPLETE for Development; Expense POST NOT STARTED.** Operator-selected scope: canonical P5B financial reads first, exact BIGINT and lifecycle/source/journal mapping, explicit Company/RLS scope and guarded production UI integration. No financial mutation, migration or new fixture. Focused automated and read-only hosted checks pass. See [checkpoint record](docs/P6D_EXPENSE_READ_FOUNDATION.md) for evidence and deferred operator UI smoke. P6C remains COMPLETE; P6E NOT STARTED; Production readiness DEFERRED. This supersedes historical P6D NOT STARTED notes below.

## Current phase checkpoint — 2026-09-23

**P6C — COMPLETE (Development); P6D/P6E — NOT STARTED; Production readiness — DEFERRED.** Slices 1–16 and final integrated boundary audit are complete after forward migration `20260923130000_p6c_closeout_grant_hardening.sql`. All 32 application tables pass the browser residual-grant audit, intended authorization/trusted grants are preserved, 41 Development migrations align, DB lint is clean, dry-run is a no-op and global integrity is 14/14 with missing/orphan 0/0. See [final closeout](docs/P6C_FINAL_CLOSEOUT.md).

This status supersedes historical P6C IN PROGRESS/NOT COMPLETE, next-slice and mutations-pending wording in dated slice narratives below. No further master slice is required and there is no Slice 17. [Pre-Demo UAT](docs/PRE_DEMO_UAT.md) remains separate/non-blocking and is not complete. Development completion is not Production readiness; the full pre-production audit and promotion gates remain deferred. No Staging/Production action, commit or push.
...
- ✅ Production Data Foundation P6A — Auth Session and Tenant Context *(verified complete; automated gates and browser/Auth smoke matrix pass; no confirmed defect)*
...
- **P6C — Master Data Async Repository Cutover (COMPLETE; Slices 1–16 and Final Closeout / Boundary Audit VERIFIED COMPLETE):** Slice 1 provides the production-only async repository/provider foundation plus read-only active-Company business profile and RLS-filtered Project queries. Explicit tenant filters, database-row mappers, scoped async states, stale-response/session protection, authenticated populated/empty tenant switching, focus revalidation, fail-closed revocation, and cleanup are verified. `/` and `/projects` prove this read slice; financial routes remain deferred holding states. Slice 2 adds read-only `/parties` and `/expense-categories` using the same scoped provider, explicit mappings, valid RLS-filtered/empty results, EN/AR states, and role-change invalidation. Build/lint/P6A/P6C checks, controlled mapper/provider checks, and hosted schema/RLS/grant inspection pass; all 27 Development migrations align and linked dry-run is a no-op. Authenticated acceptance passed: Management Viewer 6/3, Procurement 4/3 (representative DATA_ENTRY policy branch), Project Manager 0/3 with active assignment and 0/0 without, System Admin 0/0, leading-zero TRN, nullable/inactive records, Arabic contact text, Alpha→Beta isolation and unchanged focus/visibility behavior. Guarded fixture cleanup and manual Auth-user deletion are complete; global Company/settings integrity is back to 14/14. Slice 3 Accounts/Treasury READ-ONLY is VERIFIED COMPLETE: MANAGEMENT_VIEWER 8/5, hierarchy/system/party/inactive/null and leading-zero mapping, PROJECT_MANAGER assigned 0/2 with hidden GL details protected and unassigned 0/0, SYSTEM_ADMIN 0/0, Alpha→Beta isolation and focus/visibility regression passed. Guarded fixture cleanup and Auth-user removal are complete; final Development verification returns zero fixture rows and global 14/14 Company/settings integrity. See `docs/P6C_SLICE_3_ACCOUNTS_TREASURY.md`. Slice 4 Subcontracts READ-ONLY is VERIFIED COMPLETE (2026-09-13): Management Viewer/Procurement 3, assigned Project Manager 2 with hidden Party details protected, unassigned Project Manager 0, System Admin 0, exact `9007199254740993`, safe nulls, role revalidation, Alpha→Beta isolation and focus/branding regression accepted. Guarded fixture cleanup/Auth removal and final independent Development zero counts/global 14/14 settings integrity are verified. Automated checks pass; all 27 migrations align and dry-run is a no-op. See `docs/P6C_SLICE_4_SUBCONTRACTS.md`. Overall P6C remains NOT COMPLETE. P6C Slice 5 — Expense Categories MUTATION — VERIFIED COMPLETE (2026-09-13): automated/hosted and authenticated browser acceptance passed; disposable database fixture/Auth user cleanup is complete, with zero fixture counts and global Company/settings integrity 14/14. Only Accounting Admin/category.manage may create, edit metadata, deactivate or reactivate through direct RLS-governed category writes. Canonical migration `20260913120000` narrows browser columns and establishes one category provenance/update-token trigger; no financial effect or other master mutation is introduced. See `docs/P6C_SLICE_5_EXPENSE_CATEGORIES_MUTATION.md`. Accepted browser evidence covers admin list/create/edit/deactivate/reactivate, normalized duplicate rejection, stale-edit conflict, Beta read-only scope, role downgrade revalidation, tab-return regression and Arabic/RTL. Cross-tab locale convergence after refresh is a non-blocking UX observation only. Final user-run build/lint/P6A/P6C checks, diff check and focused real-secret scan passed; all 28 Development migrations align and linked dry-run is up to date. P6D financial RPC integration has not started. Later slice details appear below.
- **P6C Slice 6 — Supplier Party MUTATION — VERIFIED COMPLETE (2026-09-16).** Supplier-only create/metadata edit/deactivate/reactivate for ACCOUNTING_ADMIN and PROCUREMENT; no delete or financial mutation. Canonical migration `20260913123000` applied to Development only, aligned history and final no-op linked dry-run. Accepted evidence: 104/104 hosted PASS, competing-create uniqueness and exact-token edit concurrency PASS, authenticated role/tenant/revocation/stale-edit/focus/RTL acceptance PASS, all local gates and focused real-secret scan PASS. Row panels were moved inline and focused; real Chromium regression uses pinned development-only Playwright 1.62.1. Both cleanup scripts retain `fixture_companies`; exact-manifest cleanup left zero fixture/Auth remnants and global 14 Companies / 14 settings, zero missing/orphan settings. See `docs/P6C_SLICE_6_SUPPLIER_PARTY_MUTATION.md`. Overall P6C remains **IN PROGRESS**; P6D remains **NOT STARTED**. Slice 7 was not started at Slice 6 closure; see its subsequent completion below. No Staging/Production action; release/production readiness deferred.
- **P6C Slice 7 — Company Profile Metadata UPDATE — VERIFIED COMPLETE (2026-09-16).** Existing active Company legal_name/TRN/address/notes only; tenant-scoped company.manage remains ACCOUNTING_ADMIN and SYSTEM_ADMIN. Canonical migration `20260916120000` applied to Development only. Accepted evidence: hosted 96/96, exact-token concurrency 1/0, Supplier 104/104 and Category 70/70 regressions, Company/Supplier Chromium, build/lint/boundaries/behavior/secret scan/diff check PASS; all 30 migrations aligned, dry-run up to date, DB lint no public-schema errors. User-completed authenticated acceptance PASS: both allowed roles, denied controls, update/clear/TRN zeros, real two-tab conflict, Alpha/Beta isolation, downgrade/revocation/profile no-company, focus/tab return and Arabic/RTL/keyboard/narrow layout. Automated cleanup returned global 14/14 with no missing/orphan settings; user confirms final authenticated fixture cleanup and no stale tenant data. No credentials stored. Closure records accepted evidence without rerunning verification. P6C remains IN PROGRESS; P6D/P6E and Slice 8 NOT STARTED. No Staging/Production action or financial behavior. One local closure commit only; no push. See `docs/P6C_SLICE_7_COMPANY_PROFILE_METADATA.md` and `docs/verification/p6c-slice7/README.md`.
- **P6C Slice 8 — Project Descriptive Metadata UPDATE — VERIFIED COMPLETE (2026-09-17).** Existing Projects: name/client_name/location/contract_number/notes only, including CLOSED descriptions; Accounting Admin or assigned Project Manager under unchanged P4 RLS. No create/status/date/code/amount/financial writes. Development-only canonical migration `20260916140000` applied. Accepted evidence: hosted 108/108, concurrency 1/0, Project isolated Chromium, Company/Supplier Chromium and Category/Supplier/Company behavior regressions, build/lint (0 errors/4 existing warnings), P6A/P6C boundaries/behavior, diff/secret checks PASS; all 31 migrations aligned, dry-run up to date, DB lint clean. User authenticated acceptance PASS: Alpha admin/ACTIVE and CLOSED edits/clear/leading zeros/no protected controls; stale two-tab conflict; Alpha/Beta isolation and Beta mutation; Project Manager assigned-only edits, assignment revoke/refresh removal/restore; all five denied roles; open-edit downgrade; membership/profile revocation/restore; unchanged focus/tab return; Arabic/RTL/keyboard/focus/narrow layout. User exact-manifest cleanup/final verification: fixture Companies/Projects/memberships/assignments/browser Auth 0, global Companies/settings 14/14, missing/orphan 0/0. No verification rerun for closure. P6C IN PROGRESS; P6D/P6E NOT STARTED. No P6D/financial behavior, Staging/Production, stored credentials or Slice 9. One local closure commit only; no push. See `docs/P6C_SLICE_8_PROJECT_METADATA.md`.
- **P6C Slice 9 — Account Display Name UPDATE — VERIFIED COMPLETE (2026-09-21).** Existing Account name only, including inactive/system Accounts; ACCOUNTING_ADMIN/account.manage only. Protected configuration and Treasury/financial behavior remain excluded. Existing interrupted implementation preserved; canonical `20260917120000` applied to Development only. Hosted 99/99, concurrency 1/0, Account and directly affected Project/Company/Supplier Chromium, master behavior regressions, build/lint/boundaries/diff/secret checks PASS. Public DB lint clean; all 32 migrations aligned, final dry-run no-op. Automated cleanup/global Companies/settings 14/14, missing/orphan 0/0. User-completed authenticated acceptance PASS: admin name-only edits/normalization/protected codes and configuration, stale two-tab conflict without overwrite/retry, independent Alpha/Beta edits, full role matrix, downgrade/revocation/restores, focus/draft/keyboard/cancel/RTL/390px checks; Treasury remains read-only and financial routes unwritable. Exact guarded cleanup PASS: fixture Companies/Accounts/memberships/assignments/browser Auth 0, global 14/14, missing/orphan 0/0. Closure reran no verification. P6C IN PROGRESS; P6D/P6E NOT STARTED. Production readiness DEFERRED. No financial/P6D behavior, Staging/Production, stored credentials or Slice 10. One local closure commit only; no push. See `docs/P6C_SLICE_9_ACCOUNT_DISPLAY_NAME.md` and `docs/verification/p6c-slice9/README.md`.
- **P6C Slice 10 — Treasury Display Name UPDATE — VERIFIED COMPLETE (2026-09-21).** Existing name only for ACCOUNTING_ADMIN/treasury.manage, including inactive rows. No creation/deletion, configuration, permanent GL/Project mapping or financial behavior. Canonical `20260921120000` applied to Development only. Hosted 108/108, concurrency 1/0, Treasury/Account isolated Chromium, master behavior regressions and local gates PASS; lint 0 errors/4 existing warnings. DB lint clean; 33 migrations aligned, no-op dry-run. Exact automated fixtures removed; global Companies/settings 14/14, missing/orphan 0/0. User-completed authenticated browser acceptance and exact guarded cleanup PASS: admin ACTIVE/INACTIVE name-only edits and protected configuration; stale two-tab conflict; Alpha/Beta isolation; complete read-only/denied-role matrix; assigned Project Manager visibility and assignment revoke/restore; open-edit downgrade, membership/profile revocation/no-company; focus/tab/draft/keyboard/RTL/390px; Account name regression and unwritable financial routes. Final fixture Companies/Accounts/Treasuries/Projects/memberships/assignments/browser Auth 0; global Companies/settings 14/14, missing/orphan 0/0. Closure reran no verification. P6C IN PROGRESS; P6D/P6E NOT STARTED; production readiness DEFERRED. No financial/P6D behavior, Staging/Production, stored credentials or Slice 11. One local closure commit only; no push. See `docs/P6C_SLICE_10_TREASURY_DISPLAY_NAME.md`.
- **P6C Slice 11 — OTHER Party Display Name UPDATE — VERIFIED COMPLETE (2026-09-22).** Existing OTHER name only for ACCOUNTING_ADMIN/PROCUREMENT under unchanged party.manage; no create/status/contact/other-type or financial behavior. Development migrations `20260921130000` and forward correction `20260921133000` applied. Hosted 108/108, concurrency 1/0, Slice 11/Supplier Chromium and local gates PASS; 35 migrations aligned, dry-run no-op and DB lint clean. Authenticated browser acceptance PASS: ACTIVE/INACTIVE admin and Procurement edits, protected boundary/validation, stale conflict, Alpha/Beta isolation, role/downgrade/revocation matrix, focus/tab/keyboard and Arabic/RTL responsive real-browser verification. Earlier synthetic Playwright overflow was not reproduced manually at 394px and required no application change. Exact cleanup PASS; all fixture/Auth counts 0, global Companies/settings 14/14, missing/orphan 0/0. P6C IN PROGRESS; P6D/P6E NOT STARTED; production readiness DEFERRED. No Staging/Production or Slice 12. See `docs/P6C_SLICE_11_OTHER_PARTY_NAME.md`.
- **P6C Slice 12 — EMPLOYEE Party Display Name UPDATE — VERIFIED COMPLETE (2026-09-22).** Existing ACTIVE/INACTIVE EMPLOYEE name only, ACCOUNTING_ADMIN/party.manage. Development-only forward migration `20260922120000` applied. Hosted 115/115, concurrent exact-token 1/0, focused repository/provider and isolated Chromium including 390px EN/AR/RTL PASS; 36 migrations aligned, final linked dry-run no-op. User-completed authenticated browser acceptance PASS: ACTIVE/INACTIVE mutation, required/trim/200-vs-201 validation, controlled stale conflict without overwrite/retry, Alpha/Beta isolation, ACCOUNTANT/MANAGEMENT_VIEWER read-only, PROCUREMENT/DATA_ENTRY EMPLOYEE exclusion, PROJECT_MANAGER/SYSTEM_ADMIN no-Party behavior, open-edit downgrade, membership/Company/profile revocation/restoration, draft/keyboard/focus, real-browser 390px EN/AR/RTL long-name wrapping, type boundaries and unchanged financial/P6D routes. Exact guarded cleanup PASS; all fixture/Auth/profile counts 0, global Companies/settings 14/14, missing/orphan 0/0. P6C IN PROGRESS; P6D/P6E NOT STARTED; Production readiness DEFERRED. No Staging/Production or Slice 13. See `docs/P6C_SLICE_12_EMPLOYEE_PARTY_NAME.md` and `docs/verification/p6c-slice12/README.md`.
- **P6C Slice 13 — CUSTODIAN Party Display Name UPDATE — VERIFIED COMPLETE (2026-09-22).** Existing ACTIVE/INACTIVE CUSTODIAN name only; ACCOUNTING_ADMIN/party.manage, because unchanged P4 excludes CUSTODIAN from PROCUREMENT UPDATE while allowing its read. Forward migration `20260922130000` applied to Development only. Hosted 115/115, concurrent exact-token 1/0, focused repository/provider and isolated Custodian plus Supplier/OTHER/EMPLOYEE Chromium PASS; 37 migrations aligned, linked dry-run no-op and public DB lint clean. User-completed authenticated browser acceptance PASS: ACTIVE/INACTIVE edits, validation and 200-vs-201 boundaries, controlled stale conflict without overwrite/retry, Alpha/Beta isolation, ACCOUNTANT/MANAGEMENT_VIEWER/PROCUREMENT/DATA_ENTRY read-only behavior, PROJECT_MANAGER/SYSTEM_ADMIN no-Party behavior, open-edit downgrade, Company/membership/profile revocation/restoration, draft/focus/keyboard, real-browser 390px EN/AR/RTL long-name wrapping, Party type boundaries and unchanged custody/financial/P6D holding behavior. Exact guarded cleanup PASS; all fixture/Auth/profile counts 0, global Companies/settings 14/14, missing/orphan 0/0. P6C IN PROGRESS; P6D/P6E NOT STARTED; Production readiness DEFERRED. No Staging/Production or Slice 14. See `docs/P6C_SLICE_13_CUSTODIAN_PARTY_NAME.md` and `docs/verification/p6c-slice13/README.md`.
- **P6C Slice 14 — OWNER Party Display Name UPDATE — VERIFIED COMPLETE (2026-09-22).** Existing ACTIVE/INACTIVE OWNER name only for ACCOUNTING_ADMIN under unchanged P4 authorization. Canonical forward migration `20260922140000` applied to Development only. Hosted 115/115, concurrent exact-token 1/0, focused repository/provider and isolated Owner plus Supplier/OTHER/EMPLOYEE/CUSTODIAN Chromium PASS; build/lint/boundary checks PASS, 38 migrations aligned, linked dry-run no-op and public DB lint clean. User-completed authenticated browser acceptance PASS: ACTIVE/INACTIVE edits, validation and 200-vs-201 boundaries, controlled stale conflict without overwrite/retry, Alpha/Beta isolation, ACCOUNTANT/MANAGEMENT_VIEWER read-only, PROCUREMENT/DATA_ENTRY OWNER exclusion, PROJECT_MANAGER/SYSTEM_ADMIN no-Party behavior, open-edit downgrade, Company/membership/profile revocation/restoration, draft/focus/keyboard, real-browser 390px EN/AR/RTL long-name wrapping, Party type boundaries and unchanged owner-current-account/equity/payment/journal/P6D holding behavior. Exact guarded cleanup PASS; all fixture/Auth/profile counts 0, global Companies/settings 14/14, missing/orphan 0/0. P6C IN PROGRESS; P6D/P6E NOT STARTED; Production readiness DEFERRED. No Staging/Production or Slice 15. See `docs/P6C_SLICE_14_OWNER_PARTY_NAME.md` and `docs/verification/p6c-slice14/README.md`.
...
- **P6D — Financial Flow Cutover (IN PROGRESS):** Checkpoint 1 Expense READ Foundation COMPLETE for Development; `/expenses` uses the P5B scoped exact-money read model. Expense POST remains NOT STARTED. Specialized financial RPC integration remains subsequent work, with no hybrid financial writes. See `docs/P6D_EXPENSE_READ_FOUNDATION.md`.
...
### Future contracting operations module — sequence not assigned

**Construction Materials / Site Stores + Tool Custody** is recorded as future product scope only and does not change the current P6 → P10, Payroll/WPS, historical import, or subsequent execution order.

- **Site Materials / Stores:** material/item register; main and site stores; receipts; issues to Projects; inter-store/site transfers; returns; physical counts and controlled adjustments; on-hand quantity by store and Project; and later separately designed accounting integration where appropriate.
- **Tool Custody:** reusable tool/equipment register; issue to an employee, custodian, or Project; issue date and condition; return date and condition; and explicit in-custody, returned, damaged, and lost states.
- Tool Custody is operational custody of reusable physical assets and is distinct from MakerACC's existing financial Custody Advances. This module is intended for contracting operations, not as a full retail/distribution inventory ERP.
...
- P6B was verified complete on 2026-09-05 through `20260911120000_p6b_company_settings.sql` and forward correction `20260911123000_p6b_collision_safe_slugs.sql`, applied only to `MakerACC-Development`. Its hosted constraint/RLS matrix and complete Auth/demo browser branding/isolation matrix pass. Repeated testing found that ready state could retain `/no-company` or `/select-company`; the route table now canonically replaces obsolete Auth-state paths with `/` only after `TENANT_READY`, and the focused route re-smoke passed. Development-only cleanup removed exactly the P6B fixtures and temporary Auth/tenant browser state, preserved all 15 `cas:v1:*` keys, and left 14 Companies/14 settings with zero missing or orphan rows.
- P6C Slice 1 was implemented on 2026-09-06 without a migration: the production Auth graph now has explicit read-only Company-profile and Project repositories, mapped production models, keyed/scoped async state, and stale-result/session protection. Static P6A/P6C boundaries, build, lint, and diff checks pass; hosted and browser verification remain to be performed before this slice or P6C is marked verified complete.
- Slice 1 verification was partially repeated on 2026-09-06: automated gates passed, browser-safe configuration matched the linked Development project, anonymous Company/Project Data API reads both returned HTTP 401, and an isolated signed-out Chromium request to `/projects` settled at the production login boundary. No reusable Development test identity is configured after the prior P6A/P6B cleanup, so authorized reads, RLS role/tenant cases, populated/empty UI, switching, revocation, authenticated network/console behavior, and final Slice 1 acceptance remain manual/pending.
- The 2026-09-12 authenticated Slice 1 pass verified the mapped populated/empty states, tenant isolation and switching, P6B branding/locale, session restoration, deferred financial routes, logout, and demo isolation. It found that focus/visibility revalidation incorrectly replaced an already-valid `TENANT_READY` tree with blocking `LOADING_IDENTITY`, which unmounted P6B/P6C and redundantly reloaded their data. The narrow fix preserves the current keyed tenant tree only during background revalidation while retaining all authority queries and fail-closed transitions; explicit Company changes still invalidate the old scope. Automated build/lint/P6A/P6C/diff gates pass, but final authenticated post-fix focus/revocation/browser verification remains pending and Slice 1 is not complete.
- Follow-up runtime evidence proved that remediation incomplete: application focus plus visibility triggers could overlap, and Supabase's visibility recovery re-emitted `SIGNED_IN` for the existing user, which the Auth handler treated as a foreground login. The second correction deterministically coalesces in-flight background checks, prevents redundant same-user `SIGNED_IN` from entering blocking loading, and preserves identical ready authority state; genuine sign-in/user changes, revocation, logout, and Company-scope invalidation remain active. Automated gates pass; final authenticated post-fix browser verification remains pending.
- Final authenticated acceptance confirmed the correction: tab returns retained ready branding/master UI, issued only P6A authority reads, and did not reload settings, Company profile, or Projects. Controlled profile revocation still failed closed to `/no-company`, and restoration plus Retry rebuilt the valid two-Company selection. Exact Development cleanup left zero P6C fixture rows, restored global 14/14 Company/settings integrity with no missing/orphan settings, and the synthetic Auth user was removed. P6C Slice 1 is verified complete; overall P6C remains in progress and Slice 2 has not started.
...
*P0–P6B and the post-P5 focused financial retrospective are verified complete. P6C is in progress; P6D–P10, Payroll/WPS, and bulk historical import have not started.*
