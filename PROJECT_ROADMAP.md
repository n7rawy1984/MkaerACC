...
- **P6D — Financial Flow Cutover (IN PROGRESS):** Expense READ, Treasury-funded Expense POST, Expense Reversal, Supplier Payment, and Supplier Credit Expense POST COMPLETE for Development. Checkpoint 5 supplies the minimum bill-to-payment source through canonical `post_expense`, authoritative Expense readback, and Supplier Payment outstanding confirmation; see `docs/P6D_SUPPLIER_CREDIT_EXPENSE_POST.md`. Operator smoke is deferred/non-blocking. Corrected Replacement Expense and all other financial workflows remain NOT STARTED; no hybrid financial writes.
