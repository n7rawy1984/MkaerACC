-- Exact-manifest cleanup of only the disposable Slice 7 concurrency fixture.
begin;
do $$
declare
 actor uuid := '77100000-0000-4000-8000-000000000001'::uuid;
 fixture_companies uuid[] := array['77100000-0000-4000-8000-0000000000a1']::uuid[];
 t record; n bigint; removed integer;
begin
 if actor is null or not exists(select 1 from auth.users where id=actor and email='p6c-s7-concurrency@example.test') then raise exception 'Exact fixture Auth identity required'; end if;
 if (select count(*) from public.companies where ((id=fixture_companies[1] and code='P6C-S7-CONCURRENCY' and name='Slice 7 disposable concurrency')))<>cardinality(fixture_companies) then raise exception 'Company manifest mismatch'; end if;
 ...
 delete from public.companies where ((id=fixture_companies[1] and code='P6C-S7-CONCURRENCY' and name='Slice 7 disposable concurrency'));
 ...
 delete from auth.users where id=actor and email='p6c-s7-concurrency@example.test';
