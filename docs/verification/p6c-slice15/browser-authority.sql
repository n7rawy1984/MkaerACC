...
do $$ declare
 ...
 n integer;
begin
 if actor is null or not exists(select 1 from auth.users where id=actor and email='p6c-s15-browser@example.test')
 or not exists(select 1 from public.companies where id='85200000-0000-4000-8000-0000000000a1' and code='P6C-S15-BROWSER-A' and name='Slice 15 Alpha') then raise exception 'Exact fixture required'; end if;
 if target='company' then
 update public.companies set status=next_status where id='85200000-0000-4000-8000-0000000000a1' and code='P6C-S15-BROWSER-A';
 elsif target='membership' then
 update public.company_memberships set status=next_status where company_id='85200000-0000-4000-8000-0000000000a1' and user_id=actor;
