...
do $$ declare
 ...
 n integer;
begin
 if actor is null or not exists(select 1 from auth.users where id=actor and email='p6c-s12-browser@example.test')
 or not exists(select 1 from public.companies where id='82200000-0000-4000-8000-0000000000a1' and code='P6C-S12-BROWSER-A' and name='Slice 12 Alpha') then raise exception 'Exact fixture required'; end if;
 if target='company' then
 update public.companies set status=next_status where id='82200000-0000-4000-8000-0000000000a1' and code='P6C-S12-BROWSER-A';
 elsif target='membership' then
 update public.company_memberships set status=next_status where company_id='82200000-0000-4000-8000-0000000000a1' and user_id=actor;
