-- Exact-manifest cleanup of only the disposable Slice 14 browser fixture.
begin;
select set_config('makeracc.fixture_user','',true);
do $$
declare
 actor uuid := nullif(current_setting('makeracc.fixture_user'),'')::uuid;
 fixture_companies uuid[] := array['84200000-0000-4000-8000-0000000000a1','84200000-0000-4000-8000-0000000000a2']::uuid[];
 t record; n bigint; removed integer;
begin
 if actor is null or not exists(select 1 from auth.users where id=actor and email='p6c-s14-browser@example.test') then raise exception 'Exact fixture Auth identity required'; end if;
 if (select count(*) from public.companies where ((id=fixture_companies[1] and code='P6C-S14-BROWSER-A' and name='Slice 14 Alpha') or (id=fixture_companies[2] and code='P6C-S14-BROWSER-B' and name='Slice 14 Beta')))<>cardinality(fixture_companies) then raise exception 'Company manifest mismatch'; end if;
 ...
 if (select count(*) from public.parties where company_id=any(fixture_companies))<>7 or
 (select count(*) from public.parties where type='OWNER' and phone is null and email is null and address is null and
 ((id='84200000-0000-4000-8000-0000000000b1' and company_id=fixture_companies[1] and code='0001' and status='INACTIVE' and trn='001234567890123' and contact_person='جهة اتصال' and notes='Keep notes') or
 (id='84200000-0000-4000-8000-0000000000b2' and company_id=fixture_companies[1] and code='0002' and status='ACTIVE' and trn is null and contact_person is null and notes is null) or
 (id='84200000-0000-4000-8000-0000000000b3' and company_id=fixture_companies[2] and code='0003' and status='ACTIVE' and trn is null and contact_person is null and notes is null)))<>3 or
 ...
 ((id='84200000-0000-4000-8000-0000000000b4' and code='0004' and type='SUPPLIER' and name='Supplier regression') or
 (id='84200000-0000-4000-8000-0000000000b5' and code='0005' and type='OTHER') or
 (id='84200000-0000-4000-8000-0000000000b6' and code='0006' and type='EMPLOYEE') or
 (id='84200000-0000-4000-8000-0000000000b7' and code='0007' and type='CUSTODIAN')))<>4 then raise exception 'Party manifest mismatch'; end if;
 ...
 if (select count(*) from public.company_settings where (company_id=fixture_companies[1] and tenant_slug='p6c-s14-browser-a') or (company_id=fixture_companies[2] and tenant_slug='p6c-s14-browser-b'))<>cardinality(fixture_companies) then raise exception 'Settings manifest mismatch'; end if;
 delete from public.parties where company_id=any(fixture_companies) and id::text like '84200000-0000-4000-8000-0000000000b_';
 get diagnostics removed=row_count; if removed<>7 then raise exception 'Party delete mismatch'; end if;
 ...
 delete from public.companies where ((id=fixture_companies[1] and code='P6C-S14-BROWSER-A' and name='Slice 14 Alpha') or (id=fixture_companies[2] and code='P6C-S14-BROWSER-B' and name='Slice 14 Beta'));
 ...
 delete from auth.users where id=actor and email='p6c-s14-browser@example.test';
