...
## P6C Slice 13 — CUSTODIAN Party name — VERIFIED COMPLETE

Development migration `20260922130000` adds existing CUSTODIAN name-only UPDATE to the restrictive Party policy. ACCOUNTING_ADMIN alone may write: although PROCUREMENT has `party.manage` and reads CUSTODIAN operationally, the unchanged P4 UPDATE role policy excludes that type for PROCUREMENT. Supplier/OTHER/EMPLOYEE UPDATE and Supplier-only INSERT remain unchanged. Existing table-wide grants remain; a fixed-path SECURITY INVOKER trigger rejects authenticated CUSTODIAN metadata/status/type changes, derives DB actor and advances authenticated/trusted tokens. Forced RLS, SELECT and role mappings remain unchanged. Hosted 115/115 and concurrent exact-token 1/0 PASS; focused repository/provider and isolated Custodian/Supplier/OTHER/EMPLOYEE Chromium including 390px EN/AR/RTL PASS. Authenticated Development browser acceptance PASS across ACTIVE/INACTIVE mutation, validation, controlled stale conflict, tenant isolation, full documented role matrix, downgrade/revocation/restoration, focus/draft/keyboard, responsive EN/AR/RTL layout, protected Party types and unchanged custody/financial boundaries. Exact guarded cleanup and final verify PASS: all Slice 13 fixture/Auth/profile counts 0, global Companies/settings 14/14, missing/orphan 0/0. P6C IN PROGRESS; P6D/P6E NOT STARTED; Production readiness DEFERRED. No Staging/Production or Slice 14. See `P6C_SLICE_13_CUSTODIAN_PARTY_NAME.md`.

## P6C Slice 14 — OWNER Party name — VERIFIED COMPLETE

Development migration `20260922140000` adds existing OWNER name-only UPDATE to the restrictive Party policy. ACCOUNTING_ADMIN alone may write; PROCUREMENT retains `party.manage` but the unchanged P4 UPDATE role policy excludes OWNER. ACCOUNTANT and MANAGEMENT_VIEWER can read OWNER; PROCUREMENT and DATA_ENTRY cannot read it; PROJECT_MANAGER and SYSTEM_ADMIN see no Party rows. Forced RLS, SELECT, grants and role mappings remain unchanged; Supplier-only INSERT and prior Party UPDATE types continue. The fixed-path SECURITY INVOKER trigger guards protected fields, derives the DB actor and preserves monotonic tokens. Hosted 115/115, concurrent exact-token 1/0 and focused repository/provider/isolated Chromium PASS. Authenticated Development browser acceptance PASS across ACTIVE/INACTIVE mutation, validation, controlled stale conflict, tenant isolation, full documented role matrix, downgrade/revocation/restoration, focus/draft/keyboard, responsive EN/AR/RTL layout, protected Party types and unchanged owner-current-account/equity/payment/journal/P6D boundaries. Exact guarded cleanup and final verify PASS: all Slice 14 fixture/Auth/profile counts 0, global Companies/settings 14/14, missing/orphan 0/0. P6C IN PROGRESS; P6D/P6E NOT STARTED; Production readiness DEFERRED. No Staging/Production or Slice 15. See `P6C_SLICE_14_OWNER_PARTY_NAME.md`.

## P6C Slice 15 — SUBCONTRACTOR Party name — VERIFIED COMPLETE
...
Development migration `20260923120000` narrows authenticated Subcontract writes to `scope_of_work`, `start_date`, `expected_end_date` and `notes`. Existing `subcontract.manage` and UPDATE RLS authorize ACCOUNTING_ADMIN and PROCUREMENT; ACCOUNTANT and MANAGEMENT_VIEWER are read-only, PROJECT_MANAGER retains assigned-Project read only, and DATA_ENTRY/SYSTEM_ADMIN see no rows. ACTIVE/COMPLETED/CLOSED share the same descriptive edit boundary. Active Company/profile/membership and tenant isolation remain mandatory. Contract number, Company/Project/Subcontractor IDs, economics, retention, status and provenance are protected. Stable-ID P5 dependencies and focused count/snapshot tests prove financial documents and journals unchanged. Automated Development authorization/security checks PASS and the operator accepted the primary authenticated browser smoke. The remaining extended manual role/UI sweep is tracked in `PRE_DEMO_UAT.md` and is non-blocking for continued Development. See `P6C_SLICE_16_SUBCONTRACT_METADATA.md`.
