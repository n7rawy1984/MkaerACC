...
## P6D Checkpoint 8 Subcontractor Payment smoke

Deferred/non-blocking Development operator follow-up using an existing authorized session and disposable synthetic data: open production `/subcontractor-payments`; confirm loading, populated, empty, error and exact outstanding states; post an explicit Treasury-funded full or partial allocation across one or more posted Certificates of one Subcontract; confirm authoritative payment, allocations, journal, and refreshed outstanding after reload; then reverse a disposable payment and confirm restored outstanding and the reversal journal. Confirm ACCOUNTING_ADMIN can post/reverse, ACCOUNTANT can post but cannot reverse, MANAGEMENT_VIEWER is read-only, and all denied roles remain without payment controls. Check EN/AR/RTL and narrow-screen presentation. Never start another payment or reversal while its recovery key is unresolved. Do not start Retention or another financial workflow. No Staging/Production action is authorized.

## P6D Checkpoint 9 Subcontractor Retention Release smoke

Deferred/non-blocking Development operator follow-up using an existing authorized session and disposable synthetic posted Certificates: open production `/retention-releases`; confirm loading, populated, empty, error and exact available-retention states; post one partial or full explicit allocation across one or more Certificates from one Subcontract; confirm authoritative Release, allocations, journal and reduced remaining retention after refresh; then reverse a disposable Release with no live Retention Payment and confirm restored availability and the reversal journal. Confirm ACCOUNTING_ADMIN can post/reverse, ACCOUNTANT and MANAGEMENT_VIEWER are read-only, and all denied roles remain without Release controls. Check EN/AR/RTL and narrow-screen presentation. Never start another Release or reversal while its recovery key is unresolved. Do not start Retention Payment or another financial workflow. No Staging/Production action is authorized.
