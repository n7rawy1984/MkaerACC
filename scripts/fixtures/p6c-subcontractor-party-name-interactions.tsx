...
import { SubcontractsList } from '../../src/master/SubcontractsList';
import { useProductionMasterData } from '../../src/master/productionMasterDataContext';
...
const parties=Array.from({length:12},(_,i)=>({...audit,id:`party-${i}`,company_id:'company-a',code:`00${i}`,name:`Party ${i}`,type:'SUBCONTRACTOR',trn:'00123',contact_person:null,phone:null,email:null,address:null,notes:'Notes',status:i===11?'INACTIVE':'ACTIVE'}));
...
const projects=[{...audit,id:'project-a',company_id:'company-a',code:'P1',name:'Fixture project',client_name:null,location:null,contract_number:null,notes:null,start_date:null,expected_completion_date:null,status:'ACTIVE'}];
const subcontracts=[{...audit,id:'subcontract-a',company_id:'company-a',project_id:'project-a',subcontractor_id:'party-0',contract_number:'0001',scope_of_work:'Read-only display test',original_contract_value_minor:'0',approved_variations_minor:'0',retention_bps:0,start_date:null,expected_end_date:null,status:'ACTIVE',notes:null}];
...
const client={auth:{
  ...
  const query={
    ...
    then(done,failed){return (async()=>{
      ...
      if(kind==='update') {
        if(!active||!['ACCOUNTING_ADMIN','PROCUREMENT'].includes(role))return {data:[],error:null};
...
window.slice15={log,companies,parties,
...
function SubcontractEvidence(){
  const master=useProductionMasterData();
  const [show,setShow]=useState(false);
  return <><button type="button" onClick={()=>setShow(value=>!value)}>Show subcontract display</button>{show&&master.phase==='READY'&&<SubcontractsList subcontracts={master.subcontracts} projects={master.projects} parties={master.parties}/>}</>;
}
