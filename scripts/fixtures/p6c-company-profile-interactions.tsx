...
const client={auth:{
  ...
  const query={
    ...
    then(done,failed){return (async()=>{
      ...
      if(gate===operation && table==='companies'){gate=null;await new Promise(resolve=>{release=resolve;});}
      if(fault===operation && table==='companies'){fault=null;return {data:null,error:{code:'network'}};}
      if(kind==='update') {
        if(!active||!['ACCOUNTING_ADMIN','SYSTEM_ADMIN'].includes(role))return {data:[],error:null};
        rows=companies.filter(row=>filters.every(([key,value])=>row[key]===value));
...
window.slice7={log,companies,
  ...
  concurrent(){companies[0].legal_name='Concurrent Legal';companies[0].updated_at=`2026-09-16T00:00:00.${String(++serial).padStart(6,'0')}+00:00`;},
};
  ...
  return <>
    ...
    <SupabaseTenantSettingsProvider key={`${state.profile.userId}:${state.activeTenant.companyId}`} client={client} profile={state.profile} tenant={state.activeTenant}>
      <ProductionMasterDataProvider key={`${state.profile.userId}:${state.activeTenant.companyId}:${state.activeTenant.role}`} client={client} userId={state.profile.userId} activeCompanyId={state.activeTenant.companyId} role={state.activeTenant.role} onCompanyProfileRefreshed={syncCompanyLegalName}>
        <Routes><Route path="*" element={<TenantReadyApplication view="companyProfile"/>}/></Routes>
