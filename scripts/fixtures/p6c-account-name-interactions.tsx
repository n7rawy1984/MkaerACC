...
const accounts=Array.from({length:12},(_,i)=>({...audit,id:`account-${i}`,company_id:'company-a',code:`00${i}`,name:`Account ${i}`,account_type:'ASSET',parent_account_id:i===10?'account-11':null,requires_party:false,system_key:i===11?'INPUT_VAT':null,status:i===11?'INACTIVE':'ACTIVE'}));
accounts.push({...accounts[0],id:'account-b',company_id:'company-b',code:'B1',name:'Beta Account'});
...
window.slice9={log,companies,accounts,
  ...
  concurrent(){accounts[11].name='Concurrent Account';accounts[11].updated_at=`2026-09-17T00:00:00.${String(++serial).padStart(6,'0')}+00:00`;},
};
  ...
  return <>
    ...
    <SupabaseTenantSettingsProvider key={`${state.profile.userId}:${state.activeTenant.companyId}`} client={client} profile={state.profile} tenant={state.activeTenant}>
      <ProductionMasterDataProvider key={`${state.profile.userId}:${state.activeTenant.companyId}:${state.activeTenant.role}`} client={client} userId={state.profile.userId} activeCompanyId={state.activeTenant.companyId} role={state.activeTenant.role} onCompanyProfileRefreshed={syncCompanyLegalName}>
        <Routes><Route path="*" element={<TenantReadyApplication view="accounts"/>}/></Routes>
