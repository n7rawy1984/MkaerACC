...
for (const file of masterFiles) {
  ...
  const isSubcontractMutation = file === resolve(masterRoot, "subcontractMetadataMutations.ts");
  ...
  if (/\.(upsert|delete|rpc)\s*\(/.test(source) || (!isCategoryMutation && !isSupplierMutation && !isCompanyMutation && !isProjectMutation && !isAccountMutation && !isTreasuryMutation && !isOtherPartyMutation && !isEmployeePartyMutation && !isCustodianPartyMutation && !isOwnerPartyMutation && !isSubcontractorPartyMutation && !isSubcontractMutation && /\.(insert|update)\s*\(/.test(source))) throw new Error(`Mutation/RPC outside approved repositories: ${file}`);
  ...
  if (isSubcontractMutation && (/\.insert\s*\(/.test(source) || [...source.matchAll(/\.from\(["']([^"']+)["']\)/g)].some((m) => m[1] !== "subcontracts") || !source.includes("scope_of_work") || !source.includes("expected_end_date"))) throw new Error("Subcontract writer exceeds descriptive metadata UPDATE boundary");
