...
for(const name of readdirSync(new URL('../src/financial/',import.meta.url))) {const s=readFileSync(new URL(`../src/financial/${name}`,import.meta.url),'utf8');assert(!/\.(insert|update|delete|upsert)\s*\(/.test(s.replace('inFlight.delete(lock)', '')));assert(!/localStorage|cas:v1|service_role|AppDataContext/.test(s));if(name!=='expensePostRepository.ts')assert(!/\.rpc\(/.test(s));}
