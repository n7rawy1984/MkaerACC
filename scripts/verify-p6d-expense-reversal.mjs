...
for(const name of readdirSync(new URL('../src/financial/',import.meta.url))){const source=readFileSync(new URL(`../src/financial/${name}`,import.meta.url),'utf8');assert(!/\.(insert|update|upsert|delete)\s*\(/.test(source.replaceAll('inFlight.delete(lock)','')),name);assert(!/localStorage|cas:v1|service_role|AppDataContext|\/pages\//.test(source),name);if(!['expensePostRepository.ts','supplierCreditExpensePostRepository.ts','expenseReverseRepository.ts','supplierPaymentPostRepository.ts','supplierPaymentReverseRepository.ts','subcontractorAdvancePostRepository.ts','subcontractorAdvanceReverseRepository.ts'].includes(name))assert(!/\.rpc\(/.test(source),name);}
