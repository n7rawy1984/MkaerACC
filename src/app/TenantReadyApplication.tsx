...
import { RetentionReleasePanel } from "../financial/RetentionReleasePanel";
...
import { TreasuryNamesPanel } from "../master/TreasuryNamesPanel";
...
export default function TenantReadyApplication({ view }: { view: "expenses" | "supplierPayments" | "subcontractorAdvances" | "subcontractorCertificates" | "subcontractorPayments" | "retentionReleases" | "companyProfile" | "projects" | "parties" | "expenseCategories" | "accounts" | "treasuryAccounts" | "subcontracts" | "deferred" }) {
  const t = useT();
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-4xl px-6 py-16">
        <nav aria-label={t("productionMaster.navigation")} className="mb-6 flex flex-wrap gap-3">
          ...
          <NavLink to="/retention-releases" className={({ isActive }) => `rounded-lg border px-3 py-2 text-sm font-medium ${isActive ? "border-[var(--tenant-primary)] text-[var(--tenant-primary)]" : "border-slate-300 text-slate-600"}`}>{t("retentionReleaseRead.title")}</NavLink>
        ...
        {view === "expenses" ? <ExpenseReadPanel /> : view === "supplierPayments" ? <SupplierPaymentPanel /> : view === "subcontractorAdvances" ? <SubcontractorAdvancePanel /> : view === "subcontractorCertificates" ? <SubcontractorCertificatePanel /> : view === "subcontractorPayments" ? <SubcontractorPaymentPanel /> : view === "retentionReleases" ? <RetentionReleasePanel /> : view === "deferred" ? (
