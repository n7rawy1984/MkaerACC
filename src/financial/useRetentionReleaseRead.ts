...
import { readRetentionReleases, type RetentionReleaseSnapshot } from "./retentionReleaseRepository";

type State = { phase: "LOADING" | "ERROR" } | ({ phase: "READY" } & RetentionReleaseSnapshot);
export function useRetentionReleaseRead(client: SupabaseClient<Database>, userId: string, companyId: string, role: string, revision: number): State {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const result = await readRetentionReleases(client, companyId);
      if (!await valid()) { commit({ phase: "ERROR" }); return; }
