...
import { readSubcontractorPayments, type SubcontractorPaymentSnapshot } from "./subcontractorPaymentRepository";

type State = { phase: "LOADING" | "ERROR" } | ({ phase: "READY" } & SubcontractorPaymentSnapshot);
export function useSubcontractorPaymentRead(client: SupabaseClient<Database>, userId: string, companyId: string, role: string, revision: number): State {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const result = await readSubcontractorPayments(client, companyId);
      if (!await valid()) { commit({ phase: "ERROR" }); return; }
