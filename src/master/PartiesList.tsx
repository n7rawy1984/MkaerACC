...
export function PartiesList() {
  ...
  return <>
    <div className="mt-4 flex flex-wrap gap-2">
      {canManage && <button type="button" className={button} disabled={blocked} onClick={(event) => { returnFocus.current = event.currentTarget; setStatusTarget(null); setOtherActionId(null); setEmployeeActionId(null); setEditing("new"); }}>{t("supplierMutation.create")}</button>}
      <button type="button" className={button} disabled={phase === "PENDING" || master.otherPartyNameMutation.phase === "PENDING" || master.employeePartyNameMutation.phase === "PENDING"} onClick={refreshParties}>{t("supplierMutation.refresh")}</button>
    </div>
      ...
      <ul className="mt-5 divide-y divide-slate-200" aria-label={t("productionMaster.parties")}>
        {master.parties.map((party) => (
          <li key={party.id} className="py-4 first:pt-0 last:pb-0">
            ...
            {party.type === "OTHER" && <OtherPartyNameControl party={party} selected={otherActionId === party.id} onSelect={() => { returnFocus.current = null; setEditing(null); setStatusTarget(null); setEmployeeActionId(null); setOtherActionId(party.id); }} />}
            {party.type === "EMPLOYEE" && <EmployeePartyNameControl party={party} selected={employeeActionId === party.id} onSelect={() => { returnFocus.current = null; setEditing(null); setStatusTarget(null); setOtherActionId(null); setEmployeeActionId(party.id); }} />}
            {canManage && party.type === "SUPPLIER" && <div className="mt-3 flex flex-wrap gap-2">
              <button type="button" className={button} disabled={blocked} onClick={(event) => { returnFocus.current = event.currentTarget; setStatusTarget(null); setOtherActionId(null); setEmployeeActionId(null); setEditing(party); }}>{t("supplierMutation.edit")}</button>
              <button type="button" className={button} disabled={blocked} onClick={(event) => { returnFocus.current = event.currentTarget; setEditing(null); setOtherActionId(null); setEmployeeActionId(null); setStatusTarget(party); }}>{t(party.status === "ACTIVE" ? "supplierMutation.deactivate" : "supplierMutation.reactivate")}</button>
            </div>}
