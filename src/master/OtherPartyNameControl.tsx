...
export function OtherPartyNameControl({ party, selected, onSelect }: { party: ProductionParty; selected: boolean; onSelect: () => void }) {
  ...
  const blocked = master.subcontractorPartyNameMutation.phase === "PENDING" || master.supplierMutation.phase === "PENDING" || master.ownerPartyNameMutation.phase === "PENDING" || master.custodianPartyNameMutation.phase === "PENDING" || master.employeePartyNameMutation.phase === "PENDING" || phase === "PENDING" || phase === "ERROR" || phase === "REFRESH_ERROR";
